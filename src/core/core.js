...
function tombolditekan(e) {
    ...
    if (inputAktif.val.length < inputAktif.max) {
        ...
        if (inputAktif.limit === "0-9" && keycode >= 48 && keycode <= 57) inputAktif.val += String.fromCharCode(keycode);
        if ((inputAktif.limit === "a-z" || inputAktif.limit === "A-Z") && (keycode < 48 || keycode > 57)) inputAktif.val += String.fromCharCode(keycode);
    }
...
function air(data) {
    ...
    // Buat partikel baru
    var p = new Particle(ax, ay, speedx + (acak(10) - acak(10)) / 30, speedy + (acak(10) - acak(10)) / 30, size + (acak(10) - acak(10)) / 10, max);
    particles.push(p);
    ...
    for (var i = 0; i < particles.length; i++) {
        ...
        if (data.blok !== "none") {
            if (particles[i].y > data.blok.y - rad && particles[i].y < data.blok.y + data.blok.t + rad &&
                particles[i].x > data.blok.x && particles[i].x < data.blok.x + data.blok.l) {
                particles[i].ys *= -0.5;
                particles[i].x = ox;
                particles[i].y = oy;
            } else if (particles[i].x > data.blok.x - rad && particles[i].y < data.blok.y + data.blok.t &&
                       particles[i].y > data.blok.y && particles[i].y < data.blok.y + data.blok.t + rad) {
                particles[i].xs *= -0.5;
                particles[i].x = ox;
                particles[i].y = oy;
            }
...
function burner(data) {
    var x = data.x, y = data.y, width = data.l, height = data.t, stroke = data.warnaGaris, fill = data.warnaIsi;
...
class Popup {
    ...
    draw() {
        ...
        if (this.tutup == "ok") {
            new Tombol("OK", this.x + this.l / 2 - 40, this.y + this.t - 50, 80, 30, "bold 14pt Calibri", "white", "#12b098", "#12b098", "popup");
        }
    }
...
function keyPressHandler(e) {
    ...
    if (inputAktif.val.length < inputAktif.max) {
        if (inputAktif.limit === "*" || (inputAktif.limit === "0-9" && /\d/.test(e.key)) || (inputAktif.limit === "a-z" && /[a-zA-Z]/.test(e.key))) {
